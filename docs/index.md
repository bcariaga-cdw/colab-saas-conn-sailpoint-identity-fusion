...
| [Define](./guides/define.md)                                                          | Attribute definitions (Velocity computed attributes, unique identifiers, UUIDs, counters, customizer hooks). |
...
- Added SaaS Connectivity customizer hooks around unique attribute generation: `IdentityFusion:BeforeUniqueGeneration` and `IdentityFusion:AfterUniqueGeneration`. A customizer becomes the authoritative source for the values it returns, and the hooks are inert when no customizer is attached. See [Define](./guides/define.md).
